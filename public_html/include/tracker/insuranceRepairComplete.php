...
var $className="InsuranceRepairComplete";
  function init()
  {
    $this->insuranceRepairCompleteId = 0;
	$this->name = "";
	$this->page = 1;
	$this->start = 0;
	$this->perPage = 0;
  ...
  function Count($param)
  {
  	...
  	$query = "select count(*) as numRows from insuranceRepairComplete";
  	if (strlen($param))
  	{
  ...
  function Search($param)
  {
	 ...
	 $query = "Select * from insuranceRepairComplete";
	 if ($param)
	 {
	   $query = $query . " where ". $param;
	 }
  ...
  function Next()
  {
	 ...
	 {
	    $this->insuranceRepairCompleteId = $this->row['insuranceRepairCompleteId'];
     ...
     return($this->insuranceRepairCompleteId);
  }
  function GetById($id)
  {
	 ...
	 $param = "insuranceRepairCompleteId = $id";
  ...
  function Update()
  {
	 ...
	 $query = "Update insuranceRepairComplete set name='$name' where insuranceRepairCompleteId = $this->insuranceRepairCompleteId";
     $results = mysqli_query($link_cms,$query);
	 DebugText($query);
	 DebugText("Error:".mysqli_error($link_cms));
  }
  function Insert()
  {
     ...
     mysqli_select_db($link_cms,$database_cms);	 // Reselect to make sure db is selected
	 $name = trim(mysqli_real_escape_string($link_cms,$this->name));
	 $query = "Insert into insuranceRepairComplete (name) value ('$name')";
     $results = mysqli_query($link_cms,$query);
	 DebugText($query);
	 DebugText("Error:".mysqli_error($link_cms));
	 $this->insuranceRepairCompleteId = mysqli_insert_id($link_cms);
  }
  function Persist()
  {
  	if ($this->insuranceRepairCompleteId)
  	{
  		$this->Update();
  	}
  	else
