...
var $className="Priority";
  ...
  function Count($param)
  {
  	...
  	$query = "select count(*) as numRows from priority";
  	if (strlen($param))
  	{
  ...
  function Update()
  {
	 ...
	 $query = "Update priority set name='$name' where priorityId = $this->priorityId";
     $results = mysqli_query($link_cms,$query);
	 DebugText($query);
	 DebugText("Error:".mysqli_error($link_cms));
  }
  function Insert()
  {
     ...
     mysqli_select_db($link_cms,$database_cms);	 // Reselect to make sure db is selected
	 $name = trim(mysqli_real_escape_string($link_cms,$this->name));
	 $query = "Insert into priority (name) value ('$name')";
     $results = mysqli_query($link_cms,$query);
	 DebugText($query);
	 DebugText("Error:".mysqli_error($link_cms));
	 $this->priorityId = mysqli_insert_id($link_cms);
  }
  function Persist()
  {
  	if ($this->priorityId)
