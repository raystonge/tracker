...
var $className="MonitorServer";
  ...
  function Count($param)
  {
  	...
  	$query = "select count(*) as numRows from monitorServer";
  	if (strlen($param))
  	{
  ...
  function Update()
  {
    ...
    mysqli_select_db($link_cms,$database_cms);	 // Reselect to make sure db is selected
	  ...
	  $query = "Update monitorServer set name='$name' where monitorServerId = $this->monitorServerId";
    $results = mysqli_query($link_cms,$query);
	  DebugText($query);
	  DebugText("Error:".mysqli_error($link_cms));
  }
  function Insert()
  {
    ...
    mysqli_select_db($link_cms,$database_cms);	 // Reselect to make sure db is selected
	  $name = trim(mysqli_real_escape_string($link_cms,$this->name));
	  $query = "Insert into monitorServer (name) value ('$name')";
    $results = mysqli_query($link_cms,$query);
	  DebugText($query);
	  DebugText("Error:".mysqli_error($link_cms));
	  $this->monitorServerId = mysqli_insert_id($link_cms);
  }
  function Persist()
  {
  	if ($this->monitorServerId)
  	{
  		$this->Update();
  	}
  	else
  ...
  function Delete()
  {
    ...
    if ($this->monitorServerId <= 0)
    {
      DebugText("monitorServerId <= 0($this->monitorServerId)");
    }
    $query = "delete from monitorServer where monitorServerId=$this->monitorServerId";
    $results = mysqli_query($link_cms,$query);
    DebugText($query);
    DebugText("Error:".mysqli_error($link_cms));
  }
