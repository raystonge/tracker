...
var $className="InsurancePayment";
  function init()
  {
    $this->insurancePaymentId = 0;
	$this->name = "";
	$this->page = 1;
	$this->start = 0;
	$this->perPage = 0;
  ...
  function Count($param)
  {
  	...
  	$query = "select count(*) as numRows from insurancePayment";
  	if (strlen($param))
  	{
  ...
  function Search($param)
  {
	 ...
	 $query = "Select * from insurancePayment";
	 if ($param)
	 {
	   $query = $query . " where ". $param;
	 }
  ...
  function Next()
  {
	 ...
	 {
	    $this->insurancePaymentId = $this->row['insurancePaymentId'];
     ...
     return($this->insurancePaymentId);
  }
  function GetById($id)
  {
  ...
  function Update()
  {
	 ...
	 $query = "Update insurancePayment set name='$name' where insurancePaymentId = $this->insurancePaymentId";
     $results = mysqli_query($link_cms,$query);
	 DebugText($query);
	 DebugText("Error:".mysqli_error($link_cms));
  }
  function Insert()
  {
     ...
     mysqli_select_db($link_cms,$database_cms);	 // Reselect to make sure db is selected
	 $name = trim(mysqli_real_escape_string($link_cms,$this->name));
	 $query = "Insert into insurancePayment (name) value ('$name')";
     $results = mysqli_query($link_cms,$query);
	 DebugText($query);
	 DebugText("Error:".mysqli_error($link_cms));
	 $this->insurancePaymentId = mysqli_insert_id($link_cms);
  }
  function Persist()
  {
  	if ($this->insurancePaymentId)
  	{
  		$this->Update();
  	}
  	else
