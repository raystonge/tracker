...
var $className="InsuranceRepair";
  function init()
  {
    $this->insuranceRepairId = 0;
	$this->name = "";
	$this->page = 1;
	$this->start = 0;
	$this->perPage = 0;
  ...
  function Count($param)
  {
  	...
  	$query = "select count(*) as numRows from insuranceRepair";
  	if (strlen($param))
  	{
  ...
  function Search($param)
  {
	 ...
	 $query = "Select * from insuranceRepair";
	 if ($param)
	 {
	   $query = $query . " where ". $param;
	 }
  ...
  function Next()
  {
	 ...
	 {
	    $this->insuranceRepairId = $this->row['insuranceRepairId'];
  ...
  function Update()
  {
	 ...
	 $query = "Update insuranceRepair set name='$name' where insuranceRepairId = $this->insuranceRepairId";
     $results = mysqli_query($link_cms,$query);
	 DebugText($query);
	 DebugText("Error:".mysqli_error($link_cms));
  }
  function Insert()
  {
     ...
     mysqli_select_db($link_cms,$database_cms);	 // Reselect to make sure db is selected
	 $name = trim(mysqli_real_escape_string($link_cms,$this->name));
	 $query = "Insert into insuranceRepair (name) value ('$name')";
     $results = mysqli_query($link_cms,$query);
	 DebugText($query);
	 DebugText("Error:".mysqli_error($link_cms));
	 $this->insuranceRepairId = mysqli_insert_id($link_cms);
  }
  function Persist()
  {
  	if ($this->insuranceRepairId)
  	{
  		$this->Update();
  	}
  	else
