...
$numRows = $status->Count($param);
$pages = 1;
if ($status->perPage)
{
	$pages = ceil($numRows/$status->perPage);
}
$status->SetPage($page);
...
?>
	...
	<table class="main_rlist tableBorder">
		...
		{
			?>
		<tr class='mritem' id="<?php echo $status->statusId;?>">
			<td class="adminData"><?php echo $status->name;?></td>
			
			<td>
			<?php 
			include $sitePath."/design/actions/status.php";
			echo $htmlAction;
			?>
		    </td>
		</tr>
